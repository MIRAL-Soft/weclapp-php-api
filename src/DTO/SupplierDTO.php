 ...
 *
 * Suppliers map to the /api/v2/supplier endpoint and share the party schema
 * with customers and contacts. This DTO covers all 137 fields of the weclapp
 * OpenAPI party schema (abstractParty + party inline properties).
 *
 * Supplier-specific fields use the supplierXxx prefix. Customer-specific fields
 * (customerXxx) are present but will be null/false for pure supplier records.
 *
 * Breaking changes from earlier versions:
 * - $mobile renamed to $mobilePhone1 (correct API key)
 * - $blocked renamed to $supplierOrderBlock (correct API key)
 * - $active renamed to $supplierActive (correct API key)
 * - $vatRegistrationNumber removed — use $vatIdentificationNumber or $taxId
 * - $currencyName removed — does not exist in the party schema
 * - $paymentTermId renamed to $supplierTermOfPaymentId (correct API key)
 * - $deliveryTermId removed — does not exist in the party schema
 *
...
final class SupplierDTO extends AbstractDTO
{
     ...
     * @param string|null  $parentPartyId                               ID of the parent party.
     ...
     * @param bool         $fixedResponsibleUser                        Whether the responsible user is fixed.
     ...
     * @param string|null  $xRechnungLeitwegId                         XRechnung routing ID.
     * @param bool         $factoring                                   Whether factoring is active.
     ...
     * @param string|null  $salesPartnerDefaultCommissionFix            Default fixed commission as decimal string.
     * @param string|null  $salesPartnerDefaultCommissionPercentage     Default percentage commission as decimal string.
     ...
     * @param bool         $enableDropshippingInNewSupplySources        Whether drop-shipping is enabled.
     ...
     * @param bool         $customer                                    Whether this party is also a customer.
     * @param string|null  $customerNumber                              Customer number (if also a customer).
     * @param string|null  $customerNumberOld                           Legacy/old customer number.
     * @param bool         $customerBlocked                             Whether the customer side is blocked.
     * @param bool         $customerDeliveryBlock                       Whether delivery is blocked.
     ...
     * @param string|null  $customerInternalNote                        Internal note.
     ...
     * @param string|null  $customerNonStandardTaxId                    ID of non-standard tax rule.
     ...
     * @param bool         $supplier                                    Whether this party is a supplier (always true for SupplierDTO).
     ...
     * @param string|null  $supplierNonStandardTaxId                    ID of non-standard tax rule (purchase).
     ...
     * @param list<array>                                                                $contacts                          List of linked contact objects (raw).
    ...
    public static function fromArray(array $data): static
    {
        return new static(
            ...
            supplier:                                self::bool($data, 'supplier', true),
    ...
    /**
     * Returns the minimum purchase order amount as a float, or null if not set.
     */
    public function getSupplierMinimumPurchaseOrderAmount(): ?float
    {
        return $this->supplierMinimumPurchaseOrderAmount !== null
            ? (float) $this->supplierMinimumPurchaseOrderAmount
            : null;
    }
    ...
    /**
     * Returns true if the supplier account is active and ordering is not blocked.
     */
    public function isActive(): bool
    {
        return $this->supplierActive && !$this->supplierOrderBlock;
    }
}
