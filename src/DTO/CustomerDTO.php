 ...
 *
 * Customers map to the /api/v2/customer endpoint and share the party schema
 * with contacts and suppliers. This DTO covers all 137 fields of the weclapp
 * OpenAPI party schema (abstractParty + party inline properties).
 *
 * Customer-specific fields use the customerXxx prefix. Supplier-specific fields
 * (supplierXxx) are present but will be null/false for pure customer records.
 *
 * Breaking changes from earlier versions:
 * - $mobile renamed to $mobilePhone1 (correct API key)
 * - $blocked renamed to $customerBlocked (correct API key)
 * - $insolvent renamed to $customerInsolvent (correct API key)
 * - $active removed — use !$customerBlocked or $supplierActive instead
 ...
 * - $paymentTermId renamed to $customerTermOfPaymentId (correct API key)
 * - $deliveryTermId removed — does not exist in the party schema
 * - $responsibleUserUsername removed — does not exist in the party schema
 * - $salesChannel renamed to $customerSalesChannel (correct API key)
 *
...
final class CustomerDTO extends AbstractDTO
{
     ...
     * @param bool         $customer                                    Whether this party is a customer (always true for CustomerDTO).
     ...
     * @param bool         $supplier                                    Whether this party is also a supplier.
     ...
     * @param string|null  $supplierInternalNote                        Internal note about the supplier side.
    ...
    public static function fromArray(array $data): static
    {
        return new static(
            ...
            customer:                                self::bool($data, 'customer', true),
    ...
    /**
     * Returns true if the customer is currently blocked.
     */
