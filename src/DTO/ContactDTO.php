 ...
 *
 * Contacts map to the /api/v2/contact endpoint and share the party schema
 * with customers and suppliers. This DTO covers all 137 fields of the weclapp
 * OpenAPI party schema (abstractParty + party inline properties).
 *
 * Contacts are typically PERSON-type parties linked to an organisation via
 * $parentPartyId. Their $personRoleId and $personDepartmentId hold job role
 * and department information.
 *
 * Breaking changes from earlier versions:
 * - $mobile renamed to $mobilePhone1 (correct API key)
 * - $active removed — does not exist in the party schema
 * - $customerId renamed to $parentPartyId (correct API key)
 * - $position removed — use $personRoleId instead
 * - $department removed — use $personDepartmentId instead
 *
...
final class ContactDTO extends AbstractDTO
{
     ...
     * @param string|null  $partyType                                   Entity type: always "PERSON" for contacts.
     * @param string|null  $salutation                                  Salutation (enum: salutation, e.g. MR, MRS).
     * @param string|null  $company                                     Company name override (usually via personCompany).
     ...
     * @param string|null  $personCompany                               Name of the company this contact belongs to.
     * @param string|null  $personDepartmentId                          ID of the contact's department.
     * @param string|null  $personRoleId                                ID of the contact's role/position.
     * @param string|null  $imageId                                     ID of the contact's profile image.
     * @param string|null  $description                                 Internal description or notes.
     * @param string|null  $parentPartyId                               ID of the parent organisation (company party).
     * @param string|null  $primaryContactId                            ID of the primary contact (if this is an org).
     ...
     * @param string|null  $commercialLanguageId                        ID of the commercial language.
     ...
     * @param string|null  $supplierNumber                              Supplier number (if also a supplier).
     ...
     * @param string|null  $supplierInternalNote                        Internal note (supplier side).
